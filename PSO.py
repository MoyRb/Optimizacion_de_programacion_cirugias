...
def run_pso(inst: Instance, cfg: PSOConfig, fitness_kwargs: Optional[Dict]=None):
    rng = random.Random(cfg.seed); fitness_kwargs = fitness_kwargs or {}
    ids = [s.sid for s in inst.surgeries]; n = len(ids)
    if n == 0:
